...
void handle_print_statement(char *line, FILE *c_file) {
    ...
    if (strcmp(token, "print") == 0) {
        token = strtok(NULL, "\n");
         if (strchr(token, '(')) {
            // This is a function call, no need to modify the token further
            fprintf(c_file, "    printf(\"%%.6f\\n\", %s);\n", token);
        } else {
            // If it's just a variable or expression, print it directly
            fprintf(c_file, "    printf(\"%%.6f\\n\", %s);\n", token);
        }
    }
}
...
void assign_variable(char *line, FILE *c_file) {
    ...
    // Handle expression assignment (e.g., z <- a + b)
        ...
        // Assign expression inside main (no check needed for expressions)
        fprintf(c_file, "    %s = %s;\n", var_name, expression);
    }
}
