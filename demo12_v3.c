...
// Function to handle variable assignments (for no function case ONLY)
void handle_variable_assignment(char *line, FILE *c_file) {
    char var_name[100];
    double value;
    char expression[1024];
    if (strchr(line, '+') || strchr(line, '-') || strchr(line, '*') || strchr(line, '/')) {
        if (sscanf(line, "%s <- %[^\n]", var_name, expression) == 2) { // Handle expression assignment (e.g., z <- a + b)
            fprintf(c_file, "double %s = %s;\n", var_name, expression);
        }
    }
    else { // Handle numeric value assignment (e.g., x <- 1)
        if (sscanf(line, "%s <- %lf", var_name, &value) == 2) {
...
void compiler(FILE *ml_file, FILE *c_file) {
    ...
    while (fgets(line, sizeof(line), ml_file)) {
        ...
        while (fgets(line, sizeof(line), ml_file)) {
            ...
            //handle assignment
            if (strstr(line, "<-")) {
                handle_variable_assignment(line, c_file);  // Expressions
            }
        }
    }
