...
// Section 1: possible error checking
// Function to check for invalid indent
int indent_check(FILE *ml_file) {
    ...
    while (fgets(line, sizeof(line), ml_file)) {
        ...
        // If we're inside a function, check for indentation
        if (inside_function == 1) {
            if (line[0] != '\t' && !strstr(line, "return")) { // No more indent = outside function
                inside_function = 0;
            }
            else if (line[0] != '\t' && strstr(line, "return")) { // If the return statement doesnt have indent then return error
                fprintf(stderr, "!Error: indentation\n");
                return 1;
            }
        }

        if (inside_function == 0 && line[0] == '\t') {
            if (line[0] == '\t') {
                fprintf(stderr, "!Error: indentation\n");
                return 1;
            }
...
int parenthesis_check(FILE *ml_file) {
    char line[256];
    ...
    while (fgets(line, sizeof(line), ml_file)) {
        ...
        if ((strchr(line, '(') && !strchr(line, ')')) || (!strchr(line, '(') && strchr(line, ')'))) {
            fprintf(stderr, "!Error: incomplete parenthesis\n");
            return 1;
        }
    }
    rewind(ml_file);
...
int main(int argc, char *argv[]) {
    ...
    FILE *c_file = fopen("ml_oUtpUt.c", "w");
    if (c_file == NULL) {
    ...
    if (indent_check(ml_file) || parenthesis_check(ml_file)) {
        fclose(ml_file);
        fclose(c_file);
        return 1;
    }
    ...
    system("rm ml_oUtpUt.c ml_oUtpUt");
    return 0;
}